...
public class BladeStandEventHandler {
    ...
    public static void eventnyx(SlashBladeEvent.BladeStandAttackEvent event) {
        ...
        RequestDefinition material = new RequestDefinition(prinegorerouse.prefix("chronosn_blade"),1000,0,10,List.of(chronosn),List.of(SwordType.BEWITCHED));
    ...
    public static void eventdeligun(SlashBladeEvent.BladeStandAttackEvent event) {
        ...
        if(!slashbladeDefinitionRegistry.containsKey(NrBladeBuiltInRegistry.DELIGUN_BLADE.location()))
            return;
        if(!(event.getDamageSource().getEntity() instanceof Creeper))
            return;
        if(!(event.getDamageSource().is(DamageTypeTags.IS_EXPLOSION)))
            return;
        EnchantmentDefinition yamato = new EnchantmentDefinition(getEnchantmentID(Enchantments.POWER_ARROWS),5);
        RequestDefinition material = new RequestDefinition(prinegorerouse.prefix("aritemis_blade"),1000,0,10,List.of(yamato),List.of(SwordType.BEWITCHED));
        if(!material.test(event.getBlade()))
            return;
